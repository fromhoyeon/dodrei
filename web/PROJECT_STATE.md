...
2026-09-22 기준, 이 p5.js 구현은 가능성 테스트와 기술 검증이라는 역할을 충분히 수행한 **보존된 결과물 / 역사적 baseline**으로 취급한다.

- 현재 버전을 계속 세분화하거나 성능 최적화·세부 기능 추가를 이어가는 것은 당분간 우선순위가 아니다.
- 필요하면 이후 소폭 정리해 독립 작품 형태로 선보일 수 있다.
- 여기서 구현·검증한 기능은 이후 다른 작품이나 새로운 DODREI 구현에서 재사용할 수 있다.
- 아래의 구현 상태, 채택된 동작, 미해결 문제는 이 baseline을 복구할 수 있도록 그대로 보존한다.
- 현재 단계에서는 이 구현을 연장하는 새 작업을 시작하지 않는다. DODREI 전체의 다음 방향은 루트 `PROJECT_STATE.md`에서 관리한다.
...
다음 항목은 baseline 당시의 미해결 문제와 작업 후보를 보존한 것이다. 2026-09-22의 상위 방향 전환 이후에는 자동으로 다음 구현 우선순위를 뜻하지 않는다.
...
이 baseline 웹 구현을 다시 확인하거나 손볼 필요가 생겼을 때:
